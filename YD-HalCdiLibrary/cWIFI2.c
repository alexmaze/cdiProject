...
/******************************************************************************
*  Function : ESP8266_PowerOn
* Arguments : none
*    Action : turn on the ESP8266 module and received default information
*   Returns : none
******************************************************************************/
void ESP8266_PowerOn(void)
{
	...
	ESP8266_Reset();
    comSetBaud(COM_ESP8266,74880);
    ESP8266_WaitResponse("csum", 500);
    ESP8266_WaitResponse("1000", 500);
    comSetBaud(COM_ESP8266,921600);
    ESP8266_WaitResponse("ready", 2000);
}
...
*********************************************************************************************************
*/
uint8_t ESP8266_WaitResponse(char *_pAckStr, uint32_t _usTimeOut)
{
	uint8_t ucData;
	uint16_t pos = 0;
	uint32_t len;
	...
	{
        ulStart = BSP_GetTick();
		ulTimeOutCnt = ulStart + ((_usTimeOut*1000)*(SystemCoreClock / 1000000));		/* 使用DWT，作为超时控制 */
	}
	while (1)
	{
		if (_usTimeOut > 0)
		{
            ulCnt = BSP_GetTick() - ulStart;
			if (ulCnt > ulTimeOutCnt)
			{
				ret = 0;	/* 超时 */
				break;
			}
...
uint16_t ESP8266_ReadLine(char *_pBuf, uint16_t _usBufSize, uint16_t _usTimeOut)
{
	...
	if (_usTimeOut > 0)
	{
		ulTimeOut = BSP_GetTick() + ((_usTimeOut*1000)*(SystemCoreClock / 1000000));		/* 使用软件定时器3，作为超时控制 */
	}
	while (1)
	{
		if (BSP_GetTick() > ulTimeOut)
		{
